...
public class StudentController {
	...
	public String modifyInfo(Model model, Student entity, HttpServletRequest req) {
		HttpSession session = req.getSession();
	...
	@ResponseBody
	public String modify(Model model, Student entity, HttpServletRequest req) {
		HttpSession session = req.getSession();

		Student student = (Student) session.getAttribute("user");

		//两者不等于，则去更新代码到数据库
		if (null != entity.getPassword() && entity.getPassword() != student.getPassword()) {
			Student studentUpdate=new Student();
			studentUpdate.setId(student.getId());
			studentUpdate.setLoginname(entity.getLoginname());
			studentUpdate.setPassword(entity.getPassword());
			studentService.updateByPrimaryKey(studentUpdate);
			return "更新成功";
		}
		return "更新失败";
	}
